...
int main(int argc, char *argv[]) {
    ...
    full_code full_a = "\x1\x1\x4\x4\x4\xFF", full_b = "\0\0\xFF", full_c = "\x10\x10\x10\xFF", full_d = "\x1\x8\xFF", full_e = "\x0\xFF", full_f = "\x1\x1\x1\x40\x40\x40\x40\x40\x40\x40\x40\xFF";
    ...
    // assert(to_byte_code(read_a) == byte_a);
    // assert(to_byte_code(read_b) == byte_b);
    // assert(to_byte_code(read_c) == byte_c);
    // assert(to_byte_code(read_d) == byte_d);
    // assert(to_byte_code(read_e) == byte_e);
    // assert(to_byte_code(read_f) == byte_f);
